...
#include <locale.h> //necessário para aceitar caracteres UTF-8

/*  PROJETO Kaynã Filipe 3 SI-A-N

       Kayn's Motorcycle - Administração.
       Funções:
            Registrar motos:{Modelo,MarcaAno,Estado ,Km Rodados,Pintura,Combustível.}
            Buscar motos:{Busca moto específica no estoque}
            Motos disponíveis:{Mostra quais motos têm no estoque e a quantidade}
            Editar estoque:{Apaga moto do estoque, altera informação da moto no estoque.}
...
// Estrutura dos dado necessários para o cadastro da moto.
typedef struct {
...
// contador 
int total_motos = 0;
// O Pause(), serve para pausar a execução do programa até que o usuario pressiona "Enter".
void pause() {
    printf("\nPressione Enter para continuar...");
    getchar();
}
/*  
 A função cadastrar moto permite ao usuario cadastrar nova moto no estoque.
(coleta os dados de estrutura Moto, cria um novo nó na lista encadeada e adiciona a moto.)
*/
void cadastrarMoto(){
...
/* 
 A função listarmotos Lista todas as motos cadastradas no estoque.
( e exibe todas  as informações de cada moto armazenada na lista encadeada.)  
*/
void listarmotos(){
    system("cls");
...
/*
 A função editarMoto, permite Editar os dados de uma moto existente. 
*/
void editarMoto() {
    system("cls");
    printf("=== Editar Moto ===\n");
    ...
    printf("Moto com modelo '%s' não encontrada.\n", modeloBusca);
    pause();
}
/*
 A função removerMoto permite ao usuario remover uma moto do estoque pelo modelo.
*/
void removerMoto() {
    system("cls");
    printf("=== Remover Moto ===\n");
...
/*  
 A função buscarmoto, permite ao usuario buscar uma moto especifica cadastrada
pelo modelo e exibe suas informações se encontrada.
*/
void buscarMoto() {
    system("cls");
    printf("=== Buscar Moto ===\n");
...
/*
 A função salvarArquivo, salva o estoque atual de motos em um arquivo .txt
...
/*
  A função carregarArquivo, carrega os dados de motos salvos anteriormente
